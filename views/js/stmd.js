...
(function (exports) {
  ...
  var normalizeReference = function (s) {
    return s.trim().replace(/\s+/, " ").toUpperCase();
  };
        ...
        while (true) {
          ...
          if (
            ...
            if (first_close > 0) {
              ...
              inlines[delimpos].t =
                first_close_delims === 1 ? "Strong" : "Emph";
              inlines[delimpos].c = [
                {
  ...
  var parseEntity = function (inlines) {
    ...
    if (
      (m = this.match(
        /^&(?:#x[a-f0-9]{1,8}|#[0-9]{1,8}|[a-z][a-z0-9]{1,31});/i
      ))
    ) {
  ...
  var incorporateLine = function (ln, line_number) {
    ...
    while (
      ...
      if (indent >= CODE_INDENT) {
        ...
        (match = ln.slice(first_nonspace).match(/^`{3,}(?!.*`)|^~{3,}(?!.*~)/))
      ) {
        // fenced code block
    ...
    if (
      this.tip !== last_matched_container &&
      !blank &&
    ...
    switch (block.t) {
      case "Paragraph":
        ...
        while (
          ...
          (pos = this.inlineParser.parseReference(
      ...
      case "IndentedCode":
        block.string_content = block.strings
          .join("\n")
          .replace(/(\n *)*$/, "\n");
        break;
  ...
  var processInlines = function (block) {
    switch (block.t) {
      ...
      case "SetextHeader":
      case "ATXHeader":
        block.inline_content = this.inlineParser.parse(
          block.string_content.trim(),
          this.refmap
        );
        block.string_content = "";
        break;
    ...
    switch (block.t) {
      ...
      case "ListItem":
        return inTags(
          "li",
          [],
          this.renderBlocks(block.children, in_tight_list).trim()
        );
      case "List":
        tag = block.list_data.type == "Bullet" ? "ul" : "ol";
        attr =
          !block.list_data.start || block.list_data.start == 1
            ? []
    ...
    return {
      ...
      escape: function (s, preserve_entities) {
        if (preserve_entities) {
          return s
            .replace(
              /[&](?![#](x[a-f0-9]{1,8}|[0-9]{1,8});|[a-z][a-z0-9]{1,31};)/gi,
              "&amp;"
            )
            .replace(/[<]/g, "&lt;")
            .replace(/[>]/g, "&gt;")
